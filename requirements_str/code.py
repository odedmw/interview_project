def get_requirements(file_path, list_passed_files=[], str_modules="", list_modules=[]):
    with open(file_path, "r") as file:
        for module in file:
            if "-r " != module[:3]:
                if module.split("==")[0] not in list_modules:
                    if len(str_modules) != 0:
                        str_modules += "," + module.split("==")[0]
                        list_modules.append(module.split("==")[0])
                    else:
                        list_modules.append(module.split("==")[0])
                        str_modules += module.split("==")[0]
            else:
                ...
                if "requirements" in file_name and file_name not in list_passed_files:
                    ...
                    str_modules = get_requirements(new_path, list_passed_files, str_modules, list_modules)
